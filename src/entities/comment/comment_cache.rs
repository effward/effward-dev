    ...
    T: CommentStore + Send + Sync,
{
    async fn insert(
        ...
        self.cache
            .insert_cached(
                || async {
                    self.source
                        .insert(author_id, post_id, parent_id, content)
                        .await
                },
                build_keys,
                None,
    ...
    async fn get_count_by_post_id(&self, post_id: &u64) -> Result<i64, EntityError> {
        ...
        self.cache
            .get_cached(
                key.clone(),
                || async { self.source.get_count_by_post_id(post_id).await },
                |_| vec![key],
    ...
    async fn get_by_post_id_parent_id(
        ...
        let key = format!(
            "post_id_parent_id:{}:{:?}:{:?}:{}",
            post_id, parent_id, start_index, count
        );
        self.cache
            .get_cached(
                key.clone(),
                || async {
                    self.source
                        .get_by_post_id_parent_id(post_id, parent_id, start_index, count)
                        .await
                },
                |_| vec![key],
                Some(Duration::seconds(60)),
...
fn build_keys(comment: &Comment) -> Vec<String> {
    vec![
        build_id_key(comment.id),
        build_public_id_key(&comment.public_id),
    ]
}
