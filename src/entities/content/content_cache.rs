...
use crate::entities::{cache::Cache, utils, EntityError};

use super::{content_sql::hash_body, Content, ContentStore};
    ...
    T: ContentStore + Send + Sync,
{
    async fn insert(&self, body: &str) -> Result<Content, EntityError> {
        self.cache
            .insert_cached(
                || async { self.source.insert(body).await },
                build_keys,
                None,
            )
            .await
    ...
    async fn get_or_create(&self, body: &str) -> Result<Content, EntityError> {
        match self.get_by_body(body).await {
            // try get from cache
            Ok(content) => Ok(content),
            Err(_) => match self.insert(body).await {
                // insert into source
                Ok(content) => Ok(content),
                Err(entity_error) => match entity_error {
                    EntityError::DuplicateKey => {
    ...
    async fn get_by_body(&self, body: &str) -> Result<Content, EntityError> {
        ...
        self.cache
            .get_cached(
                key,
                || async { self.source.get_by_body(body).await },
                build_keys,
                None,
            )
            .await
...
fn build_keys(content: &Content) -> Vec<String> {
    vec![
        build_id_key(content.id),
        build_body_hash_key(content.body_hash.clone()),
    ]
}
