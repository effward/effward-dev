...
use crate::{routes::{user_context::{user_context, session_state::TypedSession, UserContextError}, utils}, entities::{comment, post}};
    ...
    match user_context::get_auth_user_entity(session, &pool).await {
        Ok(auth_user_entity) => {
            let post_entity = match ShortGuid::try_parse(&data.post_id) {
                Ok(post_public_id) => match post::get_by_public_id(&pool, *post_public_id.as_uuid()).await {
                    Ok(p) => p,
                    Err(entity_error) => {
            ...
            let parent_id = match data.parent_id.to_owned() {
                Some(parent_id) => match ShortGuid::try_parse(parent_id) {
                    Ok(parent_public_id) => match comment::get_by_public_id(&pool, *parent_public_id.as_uuid()).await {
                        Ok(p) => Some(p.id),
                        Err(entity_error) => {
            ...
            .await {
                Ok(_) => {
                    utils::success_redirect(&format!("/post/{}", data.post_id), "new comment successfully submitted")
                },
                Err(_) => {
                    utils::warning_redirect(&format!("/post/{}", data.post_id), "something went wrong submitting your comment, please try again")
                }
        ...
        Err(e) => match e {
            ...
            UserContextError::EntityError(e) =>
                utils::redirect_entity_error(e, "comment"),
